...
class HrEmployee(models.Model):
    ...
    def _compute_manual_attendance(self):
        for employee in self:
            employee.manual_attendance = employee.user_id.has_group('hr_attendance_10.group_hr_attendance') if employee.user_id else False

    @api.multi
    def _inverse_manual_attendance(self):
        manual_attendance_group = self.env.ref('hr_attendance_10.group_hr_attendance')
        for employee in self:
            if employee.user_id:
    ...
    def attendance_scan(self, barcode):
        ...
        return employee and employee.attendance_action('hr_attendance_10.hr_attendance_action_kiosk_mode') or \
            {'warning': _('No employee corresponding to barcode %(barcode)s') % {'barcode': barcode}}

    @api.multi
    def attendance_manual(self, next_action, entered_pin=None):
        self.ensure_one()
        if not (entered_pin is None) or self.env['res.users'].browse(SUPERUSER_ID).has_group('hr_attendance_10.group_hr_attendance_use_pin') and (self.user_id and self.user_id.id != self._uid or not self.user_id):
    ...
    def attendance_action(self, next_action):
        ...
        """
        self.ensure_one()
        action_message = self.env.ref('hr_attendance_10.hr_attendance_action_greeting_message').read()[0]
