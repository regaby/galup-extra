...
var GreetingMessage = Widget.extend(BarcodeHandlerMixin, {
    ...
    init: function(parent, action) {
        ...
        if(!action.attendance) {
            this.stop_listening();
            this.session.user_has_group('group_hr_attendance_user').then(function(has_group) {
                if(has_group) {
                    self.next_action = 'hr_attendance_action_kiosk_mode';
                } else {
                    self.next_action = 'hr_attendance_action_my_attendances';
                }
            });
            return;
        }

        this.next_action = action.next_action || 'hr_attendance_action_my_attendances';
        // no listening to barcode scans if we aren't coming from the kiosk mode (and thus not going back to it with next_action)
        if (this.next_action != 'hr_attendance_action_kiosk_mode' && this.next_action.tag != 'hr_attendance_kiosk_mode') {
            this.stop_listening();
        }
